...
# Tier-1 tools. Five of them, so the whole set is the rail (cap is 8) and the
# sheet renders flat — group headings are noise at this size.
...
# Five destinations renders flat; the key above is never read.
GROUPS = []

# No preset family here: every test answers a different question.
HUBS = []

# The rail carries all five tests visibly on every page, so a footer duplicate
